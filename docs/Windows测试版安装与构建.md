...
目标是 Windows 10/11 x64 用户下载安装后直接打开工作台，无需安装 Python 或执行命令。当前新增安装构建链和桌面启动窗口；实际安装包验收结果将在完成构建后补充。公开推广仍需完成联网论文流程、密钥保存与首次使用引导的发布审查。
...
当前使用浏览器作为工作台，桌面窗口负责启动和停止；并非嵌入式网页桌面壳。首版尚未实现自动升级、安装签名、全新 Windows 系统验收和云端账号计费。升级前须退出程序并备份论文数据，不能把构建通过等同于历史数据升级兼容性通过。
