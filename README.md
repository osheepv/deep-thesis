...
本轮真实模型补验继续完成环7：润色稿11,450字，环6与环7共消耗26,662 Token。环8因在线文献检索关闭而明确阻断，并提供回退入口；未执行环9、10。下一优先项是刷新后重新订阅后台作业状态，以及待审长稿的恢复展示。

2026-09-10 真实模型浏览器补验：隔离任务环6成功生成11,448字初稿，消耗10,320 Token（上限80,000），由界面确认推进到环7。修复历史卡片滞后的“继续某环”提示。发现刷新后后台作业轮询未自动恢复，需再次刷新才能显示最终状态，此问题尚待修复；不能据此宣称完整前端续作验收通过。
...
当前回归基线：**446 项 pytest 全部通过**（M16，2026-09-09），另有 **31/31 条离线学术质量规则 case 通过**。覆盖启动对账、硕博长稿恢复、作业租约与业务写入栅栏、独立 Worker、停机备份恢复、分散存储迁移、部署入口故障、源码预览包和文献重试链路。M15 增加发行包安装后的隔离验收，作为独立 CI 步骤运行，不计入 pytest 数量。
