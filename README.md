...
- M8 已验证本机 Worker 强杀后的真实租约到期恢复、3进程竞争领取24个作业、本地连接中断和作业人工重试；修复心跳/取消/重试竞态，过期 Worker 不能续租、登记完成或开始新模型调用。正式部署网络故障与业务写入执行代次校验仍待完成。详见 [M8验收报告](docs/M8后台作业进程故障与并发验收_2026-09-06.md)。
...
当前回归基线：**400 项 pytest 全部通过**（2026-09-06），另有 **31/31 条离线学术质量规则 case 通过**。M5 新增启动对账与故障矩阵，M6 增加硕士真实全流程预检和章节余量回归，M7 增加未完成章节持久化、博士长稿恢复与润色压缩守护，M8 增加进程故障、租约与并发竞态测试，M9 增加业务写入执行凭证与租约栅栏。
