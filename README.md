...
2026-09-11 Windows 可安装测试版已构建并完成临时目录实装验收：安装器约595MiB，内置 Python 3.13、CPU PyTorch 与本地检索依赖；安装后能在无源码目录的情况下启动本地 API/工作台，关闭窗口后服务正常回收。`0.1.0-beta.1` 已作为 GitHub 预发布版本提供 Windows x64 下载；当前未做代码签名，首次下载可能看到 Windows 信誉提示。见[Windows测试版安装与构建](docs/Windows测试版安装与构建.md)。

下载：打开本仓库的 GitHub Releases，选择 `v0.1.0-beta.1` 预发布版本。

2026-09-11 续作按钮补修：修正恢复结束时清除标记与刷新按钮的顺序，避免无活动作业、作业成功或失败后仍停在“恢复后台作业状态”。异步读取进度后再次核对当前论文，避免旧任务覆盖切换后的按钮。新增4种 Node 行为回归，修复前均失败，修复后 UI/预览定向测试36项通过；完整十环联网浏览器验收仍待继续，详见[M16报告](docs/M16用户全流程交互验收_2026-09-09.md)。

本轮真实模型补验继续完成环7：润色稿11,450字，环6与环7共消耗26,662 Token。环8因在线文献检索关闭而明确阻断，并提供回退入口；未执行环9、10。刷新后自动重新订阅后台作业状态的续作修复已完成并加入回归测试。

2026-09-10 真实模型浏览器补验：隔离任务环6成功生成11,448字初稿，消耗10,320 Token（上限80,000），由界面确认推进到环7。修复历史卡片滞后的“继续某环”提示，并补上刷新后重新订阅环节作业/DOCX作业的前端恢复器；恢复期间执行按钮禁用，作业完成后重建产物与恢复摘要。完整十环前端续作验收仍待真实在线文献能力开启。

2026-09-10 后台作业续作修复：初始化完成后会查询当前任务的 `PENDING` / `RUNNING` / `CANCEL_REQUESTED` 作业，按任务去重重新进入统一轮询；环节作业或 DOCX 作业结束后强制重读会话详情、任务列表和“继续上次论文”摘要，避免刷新后停在旧状态或重复提交。新增 UI 回归检查，针对性测试 32 项通过。

2026-09-10 预览升级缓存修复：统一启动器通过本地静态服务为界面资源返回 `Cache-Control: no-store`，旧条件请求返回当前完整文件，避免后续升级继续复用旧脚本。预览入口测试3项通过；浏览器推进至环6，验证真实模型不可用时模板稿被阻断且保留重试入口。环6至环10成功路径仍待真实模型浏览器验收。

2026-09-10 续作交互修复：开题评审、综述评审和大纲在待审批时，现在可随进度恢复展示产物；浏览器验证环3确认、环4评审确认、环5生成及后端重启后的大纲恢复，避免只有确认按钮却看不到待评审内容。相关测试 39 项通过，详见[M16补验记录](docs/M16用户全流程交互验收_2026-09-09.md)。

2026-09-10 补验：重启后端后，通过真实浏览器确认 M16 的环3文献重试可进入待审批状态；修复单条文献自动纳入时，执行响应缺少纳入索引导致界面显示“纳入 0 条”的问题。详见 M16 报告的补验记录。完整十环浏览器验收仍待完成。

M16 完成普通用户全流程交互验收：验证新建论文、候选选择、环1/环2人工确认、刷新后继续上次论文、环3空文献阻断、上传本地文献并重试；修复环3空结果缓存导致的重试失效，并让离线检索模式优先使用已登记的本地文献。前端上传入口改为可键盘访问的原生按钮，失败状态提供直接补充文献操作。详见[M16验收报告](docs/M16用户全流程交互验收_2026-09-09.md)。

M15 已完成 Windows 新虚拟环境首次安装验收：从 M14 ZIP 解压后通过官方 PyPI 安装，独立进程验证实际发行包中的提示词、内置模板、任务/草稿持久化与成稿下载，并验证解压包的界面/API 启动。新增 `scripts/verify_installed_runtime.py` 安装自检和 CI 发行包验收。详见[M15报告](docs/M15首次安装与发行包验收_2026-09-09.md)；本机新虚拟环境不等同于全新 Windows 系统或免安装桌面版。

M14 提供可重建的本地源码预览 ZIP 与统一启动入口，完整包含后端和前端。首次需 Python 3.12+ 及安装依赖；启动命令为 `python scripts/run_preview.py --data-dir ABSOLUTE_PATH`。只监听本机，数据保存在指定目录；见[本地预览指南](docs/本地预览指南.md)和[M14验收](docs/M14本地预览包验收_2026-09-08.md)。这不是免安装桌面版或公网发布版本。

M13 已补充真实 HTTP API 与独立 Worker 的故障恢复演练：模型请求期间强杀、自然租约到期接管、TCP 断连失败后人工重试、API 重启后保留产物和待审批状态。采用本机模型故障服务，不调用真实供应商，公网发布环境验收仍待执行。详见[M13验收报告](docs/M13部署入口故障恢复验收_2026-09-08.md)。

存储迁移现支持把**登记完整、当前表结构**的分散数据库和附件复制到新的统一目录，预检默认不发布目标，并修正任务、文献、模板和下载路径。使用 `python -m application.migrate_storage --plan ... --stopped`，通过后加 `--apply`；迁移后必须启动对账。见[存储迁移指南](docs/存储迁移指南.md)。源数据保留，历史内存登记缺失会明确阻断。

M12 已增加统一数据目录的停机备份、SHA-256 / SQLite 完整性校验和原路径恢复。使用 `python -m application.backup create|verify|restore`（安装命令 `deep-thesis-backup`）；具体参数、停机要求和回退步骤见[数据备份与恢复指南](docs/数据备份与恢复指南.md)。该阶段相关回归 21 项通过，恢复后的真实任务对账及成稿下载通过；发布环境故障演练仍待完成。见 [M12 验收报告](docs/M12停机备份与恢复验收_2026-09-08.md)。
...
- M8 已验证本机 Worker 强杀后的真实租约到期恢复、3进程竞争领取24个作业、本地连接中断和作业人工重试；修复心跳/取消/重试竞态，过期 Worker 不能续租、登记完成或开始新模型调用。正式部署网络故障验收仍待完成。详见 [M8验收报告](docs/M8后台作业进程故障与并发验收_2026-09-06.md)。
- M9 已为业务写入接入执行凭证和租约栅栏；M10 已验证独立子进程强杀、停止心跳后自然到期、同名 Worker 接管、迟到写入拒绝和重开数据库后的章节/产物保留，并修复等待数据库锁时租约时间过早计算的问题。详见 [M9验收报告](docs/M9业务写入执行凭证与租约栅栏验收_2026-09-06.md)和 [M10验收报告](docs/M10进程级章节恢复与锁等待验收_2026-09-07.md)。
...
当前回归基线：**448 项 pytest 全部通过**（M16，2026-09-10），另有 **31/31 条离线学术质量规则 case 通过**。覆盖启动对账、硕博长稿恢复、作业租约与业务写入栅栏、独立 Worker、停机备份恢复、分散存储迁移、部署入口故障、源码预览包、文献重试链路和刷新后后台作业续接。M15 增加发行包安装后的隔离验收，作为独立 CI 步骤运行，不计入 pytest 数量。
