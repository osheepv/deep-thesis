...
当前回归基线：**445 项 pytest 全部通过**（M14 全量 CI，2026-09-08），另有 **31/31 条离线学术质量规则 case 通过**。覆盖启动对账、硕博长稿恢复、作业租约与业务写入栅栏、独立 Worker、停机备份恢复、分散存储迁移、部署入口故障与源码预览包。M15 增加发行包安装后的隔离验收，作为独立 CI 步骤运行，不计入 pytest 数量。
